...
helm repo update
helm install wikijs samipsolutions/wikijs
```
...
helm install wikijs \
  --set env.TZ="America/New York" \
    samipsolutions/wikijs
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install wikijs samipsolutions/wikijs -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/samipsolutions/wikijs?modal=changelog)
