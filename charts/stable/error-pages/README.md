...
helm repo update
helm install error-pages samipsolutions/error-pages
```
...
helm install error-pages \
  --set env.TZ="America/New York" \
    samipsolutions/error-pages
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install error-pages samipsolutions/error-pages -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/samipsolutions/error-pages?modal=changelog)
