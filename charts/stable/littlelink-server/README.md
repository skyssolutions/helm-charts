...
helm repo update
helm install littlelink-server samipsolutions/littlelink-server
```
...
helm install littlelink-server \
  --set env.TZ="America/New York" \
    samipsolutions/littlelink-server
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install littlelink-server samipsolutions/littlelink-server -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/samipsolutions/littlelink-server?modal=changelog)
