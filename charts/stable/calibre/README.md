...
helm repo update
helm install calibre samipsolutions/calibre
```
...
helm install calibre \
  --set env.TZ="America/New York" \
    samipsolutions/calibre
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install calibre samipsolutions/calibre -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/samipsolutions/calibre?modal=changelog)
