...
helm repo update
helm install wireguard samipsolutions/wireguard
```
...
helm install wireguard \
  --set env.TZ="America/New York" \
    samipsolutions/wireguard
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install wireguard samipsolutions/wireguard -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/samipsolutions/wireguard?modal=changelog)
